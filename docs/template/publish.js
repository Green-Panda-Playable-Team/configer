			...
			if (!hasOwnProp.call(item, "longname")) {
				...
				if (methods.length) {
					...
					methods.forEach(function (method) {
						if (
							docdash.static === false &&
							method.scope === "static"
						)
							return;
						if (
							docdash.private === false &&
	...
	function buildMemberNavGlobal() {
		...
		if (members.globals.length) {
			...
			if (!globalNav) {
				...
				ret += "<h3>" + linkto("global", "Blocks") + "</h3>";
			} else {
				if (docdash.collapse === "top") {
					ret +=
						'<h3 class="collapsed_header">Blocks</h3><ul class="collapse_top">' +
						globalNav +
						"</ul>";
				} else {
					ret += "<h3>Blocks</h3><ul>" + globalNav + "</ul>";
				}
			}
		}
		return ret;
	...
	if (packageInfo) {
		...
		if (packageInfo.name) {
			...
			if (docdash.nameInOutputPath !== false) {
				subdirs.push(
					packageName.length > 1 ? packageName[1] : packageName[0]
				);
			}
		}
