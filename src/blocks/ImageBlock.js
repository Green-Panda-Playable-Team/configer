...
export default class ImageBlock extends FileBlock
{
    ...
    buildPreview() {
        ...
        var preview_image = this.preview_image = document.createElement('img');
    ...
    buildConfig() {
        ...
        var config_btn = this.config_btn = FileBlock.createButton('fas fa-cog');
        ...
        Utils.image.loadImage(this.originalSource.src, (img) => {
            ...
            Utils.image.convert(config, (src) => {
                ...
                if (scale !== 1) {
                    ...
                    const framesToChange = JSON.parse(JSON.stringify(this.obj.data || []));

                    for (var i in framesToChange) 
                    {
                        framesToChange[i].x = Math.floor(framesToChange[i].x * scale)
                        framesToChange[i].y =  Math.floor(framesToChange[i].y * scale)
                        framesToChange[i].width = Math.floor(framesToChange[i].width * scale)
                        framesToChange[i].height = Math.floor(framesToChange[i].height * scale)
                    }
    ...
    calculateSize() {
        ...
        let originalImageFile = this.originalSource.src + JSON.stringify(this.originalSource.data || []);
        ...
        const diff = -Math.floor(((originalSize - currentSize) / originalSize) * 100);


        if (this.image_size) {
            this.image_size.innerText = `Current size is ${currentSize.toFixed(2)} KB (${diff ? (Math.sign(diff) > 0 ? "+" : "-" ) : ""}${Math.abs(diff)}%)`
        }
            ...
            Utils.image.loadImage(file, (img) => {
                if (Utils.image.checkIfGif(img.src)) {
                    Utils.image.getGifInfo(img.src, (info_result) => {
                        ...
                        Utils.image.getSpritesheetFromGIF(img.src, info_result, (resultBase64, resultFrames) => {
                            this.obj.src = resultBase64;

                            for (var i in resultFrames) 
                            {
                                resultFrames[i].x = Math.floor(resultFrames[i].x)
                                resultFrames[i].y =  Math.floor(resultFrames[i].y)
                                resultFrames[i].width = Math.floor(resultFrames[i].width)
                                resultFrames[i].height = Math.floor(resultFrames[i].height)
                            }
