...
export default class ColorBlock extends OptionBlock {
	...
	buildColorAlpha() {
		...
		function changesColor() {
			_this.changeColorAlpha({ color: this.value, alpha: _this.alphaInput.value }, true);
		}
		function changesAlpha() {
			_this.changeColorAlpha({ color: _this.colorInput.value, alpha: this.value }, true);
		}
	...
	change(value, callCallback) {
		if (typeof this.obj.value === 'object') {
			return this.changeColorAlpha(value, callCallback);
		} else {
			return this.changeColor(value, callCallback);
		}
	}
