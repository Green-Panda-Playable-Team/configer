export default class ConfigSaver {
	...
	export() {
		...
		downloadSourceFile(
			ConfigManager.stringify(this.config),
			title + ".json"
		);
	}
	...
	import() {
		...
		Loader.file(input, function (content, options) {
			...
			if (content.indexOf("data:application/json;base64,") > -1) {
				try {
					var config_source = b64DecodeUnicode(
						content.replace("data:application/json;base64,", "")
					);
	...
	share() {
		...
		if (title.length > 2) {
			...
			u.showHint_error(
				"You must specify configuration title (min length 3)"
			);
		}
	}
	...
	_build(parent) {
		...
		html.style.cssText =
			"padding-top: 20px;bottom: 0px;display: flex;justify-content: center;align-items: center;border-top: 1px solid #c3c3c3;";
		...
		exp.innerHTML = '<i class="cicon-download"></i>';
		...
		impButton.innerHTML = '<i class="cicon-upload"></i>';
		...
		share.innerHTML = '<i class="cicon-share"></i>';
		...
		remove.innerHTML = '<i class="cicon-remove"></i>';
